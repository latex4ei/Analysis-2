...
\begin{enumerate}
	\item Bestimme $\vec x = \mat{u(t) \\ u'(t) \\ \hdots \\ u^{(n-1)}(t)}, \vec x \in \R^n$ 
	\item Erhalte folgende Gleichungen\\
		\subitem 1) $f = 0$
		\subitem 2) $x_1'(t) = x_2(t)$
		\subitem $\hdots$
		\subitem n) $x_{n-1}'(t) = x_n(t)$
	\item Stelle das DGL-System $x' = Ax$ auf
\end{enumerate}
...
\subsubsection{Matrix diagonalisierbar}
\begin{enumerate}
...
Bei komplexen EW $\lambda = \lambda_{\text{re}}+ \i \lambda_{im}$ und $\vec v = \vec v_{\text{re}} + \i \vec v_{\text{im}}$: \\
$\sum_{i=1}^n c_{\text{re,i}} \e^{t\lambda_{\text{re,i}}}[\cos{(t\lambda_{\text{im, i}})}\vec v_{\text{re,i}} - \sin{(t\lambda_{\text{im,i}})}\vec v_{\text{im,i}}]+c_{\text{im,i}}\e^{t\lambda_{\text{re,i}}}[\cos{(t\lambda_{\text{im,i}})}\vec v_{\text{im,i}}+\sin{(t\lambda_{\text{im,i}})}\vec v_{\text{re,i}}]$

\paragraph{Alternative}
TODO %TODO mit Diagonalmatrix
\begin{enumerate}
	\item Bestimme n EW $\lambda_i$ und deren EV $\vec v_i$ von $\ma A$
	\item $S = \mat{\vec v_1 & \hdots & \vec v_n}$
\end{enumerate}
...
\begin{enumerate}
	...
	\item Bestimme $F(t)$
	\item Löse homogene DGL mit $g(t) = 0$ (siehe \ref{sec:sep-dgl}) $\Rightarrow u_{hom}(t)$
...
$u_1 = e^{\tilde \lambda x}, \hdots, u_i = t^i e^{ \lambda t}$\\
		\item Ist $ \lambda$ eine $k$-fache konjugiert komplexe NST $\lambda = a + \i b$:\\
$u_1 = e^{at} \cos (bt),\; u_2 = e^{at} \sin (bx)$ \\
bzw. $u_i = t^i e^{at} \sin (bt)$ und $u_{i+1} =  t^i e^{at} \cos (bt)$
	...
	\end{itemize}
	\item $u(t) = c_1 u_1 (x) + \ldots + c_n u_n (t)$ mit $c_1, \ldots c_n \in \R$ ist Lösung der DGL
	\item Bestimme $c_i$ durch Anfangsbedingungen
\end{enumerate}

\subsection{Lösen von allgemeinen DGL-Systemen}
$\boxed{\dot {\vec u}(t) = \ma A(t) \bdot \vec u(t) + \vec f(t)}$ mit $u(t_0) = u_0$\\
Lösung: $u(t) = \e^{t-t_0)\ma A}(u_0 + \int_{t_0}^t e^{-(s-t_0)\ma A}f(s)ds)$\\
